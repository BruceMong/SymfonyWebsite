...
(function() {
    ...
    ScheduleTemplate.prototype.showDataEvent = function(event) {
        start = event.target.attributes[0].nodeValue;
        start = start.split(':');

        end = event.target.attributes[1].nodeValue;
        end = end.split(':');
        ...
        var f = new Date();
        d.setHours(start[0]);
        d.setMinutes(start[1]);

        f.setHours(end[0]);
        f.setMinutes(end[1]);

        duree = document.getElementById('duree').innerHTML;
        f = subMinutes(f, parseInt(duree));




        select = document.getElementById('choose-horaire');


        while (d.getMinutes() % 5 != 0) {
            d = addMinutes(d, 1);
        } //arrondissement des minutes à x5 ou x0 pour pas boucler infini après


        var opt = document.createElement('option');
        opt.value = d.getHours() + ":" + d.getMinutes();
        opt.innerHTML = d.getHours() + ":" + formatMinutes(d.getMinutes()) + " h";
        select.appendChild(opt);

        var i = 0;
        var g = false;

        while (d.getHours() != f.getHours() || d.getMinutes() != f.getMinutes()) {
            i++;
            g = false;
            d = addMinutes(d, 5);
            console.log(i);
        ...
        date = event.target.attributes[2].nodeValue; //je met la date qui est dans le data-event dans le imput pour qu'il s'envoie lors du post

        //console.log(event);

        document.getElementById('dataDate').value = date.split('=')[1];

        this.updateHoraireEnd();
    };
    ScheduleTemplate.prototype.updateHoraireEnd = function() {
        var horSelected = $('#choose-horaire').val();
        horSelected = horSelected.split(':');
        var dureeHour = time_convert(parseInt(duree));
        dureeHour = dureeHour.split(':');

        horSelected[0] = parseInt(horSelected[0]) + parseInt(dureeHour[0]);
        horSelected[1] = parseInt(horSelected[1]) + parseInt(dureeHour[1]);

        document.getElementById('heureFin').innerHTML = "à    " + horSelected[0] + ":" + formatMinutes(horSelected[1]) + " h";
        document.getElementById('dataHoraireEnd').value = horSelected[0] + ":" + formatMinutes(horSelected[1]);
    };

    ScheduleTemplate.prototype.deleteOpt = function() {
        select = document.getElementById('choose-horaire');
    ...
    ScheduleTemplate.prototype.initEvents = function() {
        ...
        for (var i = 0; i < this.singleEvents.length; i++) {
            ...
            this.singleEvents[i].addEventListener('click', function(event) {
                ...
                self.showDataEvent(event);
                $('cd-schedule-modal__name').animate({ scrollTop: 0 }, 'fast');
                if (!self.animating) self.openModal(this.getElementsByTagName('a')[0]);
            });
            select.addEventListener('click', function() {
                self.updateHoraireEnd();
            });
        }
        //close modal window
